...
This is my working copy of https://github.com/victor11235/Neural-Shadow-QST (AGPL-3.0) for a research project on reproducing NSQST and moving it towards real quantum hardware. My additions are in `runs/`: scripts for the two demos (`run_ghz6.py`, `run_ghz40.py`), a plotting script, saved training curves in `runs/results/` and figures in `runs/figures/`. The only change to the original code is that the `qusetta` import in `Helper_fun_new.py` is optional. The original README follows.
